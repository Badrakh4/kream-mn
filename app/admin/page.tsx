  ...
  useEffect(() => {
    const loadRequests = window.setTimeout(async () => {
      const { data, error: loadError } = await supabase
        .from("orders")
        .select("id, created_at, product_name, size, price, customer_name, phone, messenger, facebook_profile_url, messenger_username, instagram_username, status")
        .order("created_at", { ascending: false });

      if (loadError) {
        console.log("FULL ERROR", loadError);
        alert(JSON.stringify(loadError, null, 2));
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-[#f5f5f0]">
      ...
      <div className="mx-auto max-w-[1440px] px-5 py-10 sm:px-8 lg:px-12 lg:py-16">
        ...
        <div className="mt-8 overflow-x-auto border border-white/10 bg-[#0c0c0c]"><table className="w-full min-w-[1300px] border-collapse text-left"><thead><tr className="border-b border-white/10 text-[10px] font-bold uppercase tracking-[0.16em] text-white/40"><th className="px-5 py-4">Product</th><th className="px-5 py-4">Size</th><th className="px-5 py-4">Price</th><th className="px-5 py-4">Customer Name</th><th className="px-5 py-4">Phone</th><th className="px-5 py-4">Status</th><th className="px-5 py-4">Contact</th></tr></thead><tbody>{loaded && requests.map((request) => { const safeStatus = request.status ?? "pending"; return <tr key={request.id} className="border-b border-white/5 last:border-0 transition-colors hover:bg-white/[0.03]"><td className="px-5 py-5"><p className="text-sm font-semibold text-white">{request.product_name}</p><p className="mt-1 text-[10px] uppercase tracking-[0.14em] text-white/30">{new Date(request.created_at).toLocaleDateString()}</p></td><td className="px-5 py-5 text-sm text-white/70">EU {request.size}</td><td className="px-5 py-5 text-sm font-bold text-[#d7ff3f]">{request.price}</td><td className="px-5 py-5 text-sm text-white/80">{request.customer_name}</td><td className="px-5 py-5 text-sm text-white/60">{request.phone}</td><td className="px-5 py-5"><select aria-label={`Update status for ${request.product_name}`} value={safeStatus} onChange={(event) => updateStatus(request.id, event.target.value as OrderStatus)} className={`border bg-transparent px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.12em] outline-none ${statusStyles[safeStatus]}`}>{ORDER_STATUSES.map((status) => <option className="bg-[#0c0c0c] text-white" key={status} value={status}>{status[0].toUpperCase() + status.slice(1)}</option>)}</select></td><td className="px-5 py-5"><ContactActions facebookProfileUrl={request.facebook_profile_url ?? null} messengerUsername={request.messenger_username ?? request.messenger ?? null} phone={request.phone} /></td></tr>; })}</tbody></table>{loaded && !error && requests.length === 0 && <p className="px-5 py-12 text-center text-sm text-white/40">No order requests yet.</p>}{!loaded && <p className="px-5 py-12 text-center text-sm text-white/40">Loading requests...</p>}</div>
      </div>
    </main>
  );
}
